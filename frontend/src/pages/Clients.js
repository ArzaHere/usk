...
const Clients = () => {
  ...
  const fetchTestimonials = async () => {
    try {
      ...
      // Filter only testimonials with rating 4 or 5 for display
      const topTestimonials = response.data
        .filter(t => t.rating >= 4)
  ...
  const getColorByIndex = (index) => {
    const colors = ['blue', 'green', 'purple', 'pink', 'indigo', 'orange'];
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-20 max-w-5xl mx-auto">
          ...
          <div className="group bg-gradient-to-br from-green-500 to-emerald-600 text-white rounded-2xl p-8 text-center shadow-xl hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 animate-fade-in-up relative overflow-hidden" style={{ animationDelay: '100ms' }}>
            <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full -translate-y-16 translate-x-16 group-hover:scale-150 transition-transform duration-700"></div>
            <div className="relative z-10">
              <div className="text-5xl font-bold mb-3">{testimonials.length}+</div>
              <div className="text-lg font-medium opacity-90">Testimoni Positif</div>
              <div className="mt-4 w-16 h-1 bg-white/30 mx-auto rounded-full"></div>
            </div>
          </div>
            ...
            <div className="relative z-10">
              <div className="text-5xl font-bold mb-3">
                {testimonials.length > 0 
                  ? Math.round(testimonials.reduce((acc, t) => acc + t.rating, 0) / testimonials.length * 20)
                  : 98}%
              </div>
        ...
        {testimonials.length > 0 && (
          <div className="mt-20">
              ...
              {testimonials.slice(0, 3).map((testimonial, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Testimonial Text */}
                    <p className="text-gray-600 mb-6 leading-relaxed relative z-10 line-clamp-4">
                      "{testimonial.content}"
                    </p>
                    ...
                    <div className="flex items-center relative z-10">
                      {testimonial.image_url ? (
                        <img 
                          ...
                          className="w-12 h-12 rounded-full object-cover mr-4 shadow-lg group-hover:scale-110 transition-transform duration-500"
                        />
                      ) : (
                        <div className={`w-12 h-12 bg-gradient-to-br from-${color}-500 to-${color}-600 rounded-full flex items-center justify-center text-white font-bold mr-4 shadow-lg group-hover:scale-110 transition-transform duration-500`}>
                    ...
                    {/* Bottom Accent */}
                    <div className={`absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-${color}-500 to-${color}-600 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left`}></div>
                  </div>
                );
              })}
            </div>
                        ...
                        <div className="bg-white rounded-2xl shadow-lg p-8 relative overflow-hidden">
                          ...
                          <div className="flex items-center relative z-10">
                            {testimonial.image_url ? (
                              ...
                              <div className={`w-12 h-12 bg-gradient-to-br from-${color}-500 to-${color}-600 rounded-full flex items-center justify-center text-white font-bold mr-4 shadow-lg`}>
                                {testimonial.name.charAt(0).toUpperCase()}
                              </div>
                            )}
                            <div>
            ...
            {/* Show More Testimonials Link */}
            {testimonials.length > 3 && (
              <div className="text-center mt-12">
                <a 
                  href="/testimonials" 
                  className="inline-flex items-center px-8 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl font-semibold hover:from-blue-700 hover:to-indigo-700 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105"
                >
          ...
          <div className="mt-20">
            <div className="text-center py-12 bg-white rounded-2xl shadow-lg max-w-2xl mx-auto">
              ...
              <p className="text-gray-600">
                Testimoni dari klien kami akan muncul di sini
              </p>
            </div>
          </div>
        )}
